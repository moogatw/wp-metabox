...
class Metabox {
	...
	public function addColor( $args, $repeater = false ) {
		$field = array_merge( array( 'type' => 'color' ), $args );
		if ( ! $repeater ) {
			$this->_fields[] = $field;
		} else {
			return $field;
	...
	public function show_field_number( $field, $meta ) {
		$this->before_field( $field );
		echo sprintf(
			'<input type="number" placeholder="%5$s" class="%1$s" id="%2$s" name="%2$s" value="%3$s" max="%4$s">',
			esc_attr( $this->get_block_element_class_with_namespace( $field['type'] ) ),
			esc_attr( $field['id'] ),
			esc_attr( $meta ),
			esc_attr( $field['max'] ),
	...
	public function show_field_color( $field, $meta ) {
		$this->before_field( $field );
		echo sprintf(
			'<input type="color" class="%1$s" id="%2$s" name="%2$s" value="%3$s">',
			esc_attr( $this->get_block_element_class_with_namespace( $field['type'] ) ),
			esc_attr( $field['id'] ),
			esc_attr( $meta )
		);
	...
	public function show_field_radio( $field, $meta ) {
		...
		foreach ( $field['options'] as $key => $value ) {
			if($value === 'left' || $value === 'center' || $value === 'right'){
					echo sprintf(
						'
		                    <label class="radio" for="%1$s"><i class="dashicons dashicons-editor-align%2$s"></i>
	...
	public function get_repeated_block( $field, $meta, $index, $isTemplate = false ) {
		...
		$item_title  =  @$meta['mooga_repeater_main_title']; //重覆器標題
		...
		echo sprintf(
			...
			esc_attr( 
				sprintf( '%s %s %s', $this->get_block_element_class( 'repeated-header', 'title' ), $this->get_element_class_with_namespace( 'col' ), $this->get_element_class_with_namespace( 'col-6' ) ) 
			),
			// esc_html( sprintf( '%s ' . ( $isTemplate ? '%s' : '%d' ), $field['single_label'], ( $isTemplate ? self::REPEATER_ITEM_NUMBER_PLACEHOLDER : $index + 1 ) ) ),

			esc_html( sprintf( '%s ' ,$item_title) ),
